...
func TestGitAnnexViews(t *testing.T) {
	...
	onGiteaRun(t, func(t *testing.T, u *url.URL) {
		// create a public repo
		ctx := NewAPITestContext(t, "user2", "annex-template-render-test")
		require.NoError(t, doCreateRemoteAnnexRepository(t, u, ctx, false))
...
func doAnnexInitTest(remoteRepoPath string, repoPath string) (err error) {
	_, _, err = git.NewCommandNoGlobals("annex", "init", "cloned-repo").RunStdString(&git.RunOpts{Dir: repoPath})
	if err != nil {
...
func doInitAnnexRepository(repoPath string) error {
	...
	// 'git annex init'
	err = git.NewCommandNoGlobals("annex", "init", "test-repo").Run(&git.RunOpts{Dir: repoPath})
	if err != nil {
		return err
	}
...
func withAnnexCtxHTTPPassword(t *testing.T, u *url.URL, ctx APITestContext, callback func()) {
	...
	_, _, err = git.NewCommandNoGlobals("config", "--global", "credential.helper", git.CmdArg(fmt.Sprintf("store --file=%s", creds))).RunStdString(&git.RunOpts{})
	require.NoError(t, err)

	defer (func() {
		// reset
		if hasOriginalCredentialHelper {
			_, _, err = git.NewCommandNoGlobals("config", "--global", "credential.helper", git.CmdArg(originalCredentialHelper)).RunStdString(&git.RunOpts{})
		} else {
