...
func Pointer(te *git.TreeEntry) (string, error) {
	...
	_, examineStderr, err := git.NewCommandNoGlobals("annex", "examinekey", pointer).RunStdString(&git.RunOpts{Dir: te.Repo().Path})
	if err != nil {
...
func ContentLocation(te *git.TreeEntry) (string, error) {
	...
	contentLocation, _, err := git.NewCommandNoGlobals("annex", "contentlocation", pointer).RunStdString(&git.RunOpts{Dir: repoPath})
	if err != nil {
